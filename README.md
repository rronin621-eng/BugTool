...
普通用户请直接从 [Releases 页面](../../releases) 下载最新的 `BUG工具-x.x.x-arm64.dmg` 安装包，**不要下载源码压缩包**（源码需要自行搭建开发环境才能运行）。

安装步骤：

1. 双击 `.dmg`，把「BUG工具」拖入「应用程序」文件夹
2. 应用未经 Apple 签名，首次打开需解除拦截（二选一）：
   - **方式一**：右键点击「BUG工具」→「打开」→ 弹窗中再点「打开」
   - **方式二**：若提示「已损坏，无法打开」，在终端执行：`xattr -cr /Applications/BUG工具.app`
3. 启动后会自动弹出**权限引导窗口**，按提示操作：
   - 点击每项权限的「打开设置」，在系统设置中勾选「BUG工具」
   - 勾选「屏幕录制」后需点击引导窗口的「重启应用」才能生效
   - 全部授权后点「继续」即可使用

> ⚠️ **macOS 用户必读：从源码运行前请完成以下设置**（从 Releases 下载 DMG 安装的用户只需看上方「下载安装」一节）
>
